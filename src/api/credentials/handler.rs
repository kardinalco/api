...
impl CredentialsRoute {
    ...
    async fn list_credentials(_session: AuthSession, _db: DbReq) -> Result<String, Error> {
        Ok("".to_string())
    }
    
    #[instrument(skip(session, db))]
    async fn create_credentials(session: AuthSession, db: DbReq, body: Dto<CredentialCreate>) -> Result<CreateCredentialResponse, Error> {
        session.enforce(Resource::Credential(CredentialPermission::Create)).await?;
        let result = CredentialDomain::create_credentials(&db.into_inner(), &session.user.id, body.into_inner()).await?;
        Ok(CreateCredentialResponse(Credential::from_model(result)))
    }

    #[instrument(skip(session, db))]
    async fn get_credentials(session: AuthSession, db: DbReq, id: Path<String>) -> Result<GetCredentialResponse, Error> {
        ...
        let result = CredentialDomain::get_credentials(&db.into_inner(), &id).await?;
        Ok(GetCredentialResponse(Credential::from_model(result)))
    }

    #[instrument(skip(session, db))]
    async fn update_credentials(session: AuthSession, db: DbReq, id: Path<String>, body: Dto<CredentialUpdate>) -> Result<UpdateCredentialResponse, Error> {
        ...
        let result = CredentialDomain::update_credentials(&db.into_inner(), &session.user.id, &id.into_inner(), body.into_inner()).await?;
        Ok(UpdateCredentialResponse(Credential::from_model(result)))
    }

    #[instrument(skip(session, db))]
    async fn delete_credentials(session: AuthSession, db: DbReq, id: Path<String>) -> Result<DeleteCredentialResponse, Error> {
        ...
        let result = CredentialDomain::delete_credentials(&db.into_inner(), &session.user.id, &id.into_inner()).await?;
