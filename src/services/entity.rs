...
impl<T: EntityTrait> WithCreatedByUser for Select<T> {
    fn with_created_user<E: Related<Entity>>(self) -> Self {
        ...
        self.join_as(LeftJoin, E::to(), alias_created_by.clone())
            .expr_as(Expr::col((alias_created_by.clone(), Column::FirstName)), "created_by_firstname")
            .expr_as(Expr::col((alias_created_by.clone(), Column::LastName)), "created_by_lastname")
            .expr_as(Expr::col((alias_created_by.clone(), Column::Email)), "created_by_email")
            .expr_as(Expr::col((alias_created_by.clone(), Column::DeletedAt)), "created_by_deleted_at")
    }
}
