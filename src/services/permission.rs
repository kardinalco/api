...
impl Permission {
    ...
    async fn get(&self) -> Result<Vec<Role>, Error> {
        ...
        match conn.exists::<_, i32>(KEY).await? {
            1 => match self.get_permission_from_cache().await {
                Ok(roles) => Ok(roles),
    ...
    pub async fn enforce_or(&self, user: &entity::user::Model, resource: Vec<(String, String)>) -> Result<(), Error> {
        let roles = self.get().await?;
        let role = RoleDomain::get_user_role(&self.db, user).await?;
        let permissions = self
            .get_role_permissions(&roles, &role)
            .ok_or(self.build_or_error(&user, &resource))?;
        resource.iter().any(|(obj, action)| {
            if let None = permissions.iter().find(|p| p.resource == *obj && (p.action == *action || p.action == "all")) {
                return false;
            }
            true
        }).then(|| ()).ok_or(self.build_or_error(&user, &resource))?;
        Ok(())
    }
    ...
    fn build_or_error(&self, _user: &entity::user::Model, vec: &Vec<(String, String)>) -> Error {
        Error::Auth(AuthenticateError::Unauthorized(format!(
            "required one of {:?} to perform that !",
            vec
        )))
    }
